...
const TASK_SELECT = `
  ...
  (SELECT json_build_object('id', u.id, 'name', u.name, 'email', u.email)
     FROM users u WHERE u.id = t.owner_id)   AS owner,
  t.title,
  t.description,
  t.status,
    ...
    storedNames.map(async (storedName) => {
      try {
        ...
        if (error.code !== 'ENOENT') {
          logger.error({ err: error, storedName }, 'Не удалось удалить файл вложения');
        }
      }
    })
  );
...
const taskRepository = {
   ...
   * ownerId — ограничение «только задачи этого пользователя» (null — все задачи).
   */
  async list({ status = 'all', q = '', sort = 'dueDate', ownerId = null } = {}) {
  ...
  async create(ownerId, values, files = []) {
    return withTransaction(async (client) => {
      const { rows } = await client.query(
        `INSERT INTO tasks (owner_id, title, description, status, due_date)
         VALUES ($1, $2, $3, $4, $5::date)
         RETURNING id`,
        [ownerId, values.title, values.description, values.status, values.dueDate]
      );
